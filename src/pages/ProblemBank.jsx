    ...
    <div className="space-y-6 animate-fadeIn">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-800 pb-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Simpan temuan masalah lapangan secara instan agar tidak hilang.
          </p>
        </div>
        <a
          href="#form-masalah"
            ...
            <tbody className="divide-y divide-slate-800 text-sm">
              {problems.length === 0 ? (
                <tr>
                  ...
                  >
                    Belum ada masalah desa yang dimasukkan ke bank data. Mulai
                    survei lapangan!
                  </td>
                </tr>
              ) : (
                        ...
                        <div className="flex items-center justify-end gap-1">
                          {p.status === "Pending" && (
                            <>
                              <button
                                onClick={() =>
                                  updateProblemStatus(p.id, "Disetujui")
                                }
                              ...
                              <button
                                onClick={() =>
                                  updateProblemStatus(p.id, "Ditolak")
                                }
      ...
      >
        <h3 className="font-bold text-white text-lg flex items-center gap-2">
          <FolderPlus className="h-5 w-5 text-indigo-400" /> Form Input Masalah
          Desa (Saat Survei)
        </h3>
        <p className="text-xs text-slate-400">
          Tim Litbang atau siapa pun anggota tim yang sedang wawancara bisa
          menginputkan data ke sini.
        </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <select
                ...
                onChange={(e) =>
                  setNewProblem({ ...newProblem, severity: e.target.value })
                }
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-indigo-500"
              >
            ...
            <div className="space-y-1.5">
              ...
              <input
                ...
                onChange={(e) =>
                  setNewProblem({ ...newProblem, reporter: e.target.value })
                }
                className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm text-slate-100 focus:outline-none focus:border-indigo-500"
              />
            </div>

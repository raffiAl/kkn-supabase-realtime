...
export default function TaskBoard({
  ...
  const TaskCard = ({ t }) => (
    <div
      className={`bg-slate-900 border border-slate-800 p-4 rounded-xl space-y-3 shadow-md ${t.status === "Selesai" ? "opacity-70" : ""}`}
    >
      <div className="space-y-1">
        <h4
          className={`text-sm font-bold text-slate-200 ${t.status === "Selesai" ? "line-through text-slate-300" : ""}`}
        >
          {t.title}
        </h4>
        <div
          className={`text-xs ${t.status === "Selesai" ? "text-slate-500" : "text-slate-400"}`}
        >
        ...
        <div className="flex gap-1">
          {isAdmin ? (
            ...
            <span
              className={
                t.status === "Selesai"
                  ? "text-emerald-500 flex items-center gap-0.5"
                  : "text-slate-500"
              }
            >
        ...
        <div className="bg-slate-900/50 border border-slate-850 rounded-2xl p-5 space-y-4">
          <div className="flex items-center justify-between border-b border-slate-800 pb-2">
            <h3 className="font-bold text-indigo-400 flex items-center gap-2">
              <Clock className="h-4 w-4 text-indigo-400 animate-pulse" /> Sedang
              Berjalan
            </h3>
